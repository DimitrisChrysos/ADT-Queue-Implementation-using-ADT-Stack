...
SetNode init_set_from_vector_with_balanced_tree(Set set, Vector values, VectorNode start, VectorNode end, CompareFunc compare)  {
	...
	if (set->size > 0)  {
		bool j = true;
		Pointer start_temp = set_node_value(set, first_node);
		SetNode moving_node = first_node;
		int height_counter = 0;
		while (j == true)  {

			height_counter++;

			int temp = set->compare(start_temp, value_of_middle);
			if (temp == 0)  {
				j = false;
				root->height = height_counter;
			}
			else if (temp > 0)  {
				if (moving_node->left != NULL)  {
					moving_node = moving_node->left;
					start_temp = set_node_value(set, moving_node->left);
				}
				else  {
					root->height = height_counter;
					j = false;	
				...
				if (moving_node->right != NULL)  {
					...
					start_temp = set_node_value(set, moving_node->right);
				}
				else  {
					root->height = height_counter;
					j = false;	
